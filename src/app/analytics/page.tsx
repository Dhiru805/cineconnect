...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    fetch("/api/auditions", { credentials: "include" })
      .then((r) => r.ok ? r.json() : null)
      .then((d) => {
        if (!d) return;
        const list: IAudition[] = d.auditions || [];
        setAuditions(list);
        setAuditionStats({
          total: list.length,
              ...
              {activeTab === "earnings" && (
                <div className="space-y-5">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <StatCard icon={IndianRupee} label="Total Earnings" value="—" sub="Requires contracts" color="text-green-400" />
                    <StatCard icon={Briefcase} label="Active Contracts" value="—" sub="Check Hiring tab" color="text-primary" />
                    <StatCard icon={Users} label="Clients Worked With" value="—" sub="Post-project" color="text-blue-400" />
                  </div>
                  ...
                  <div className="p-8 rounded-2xl border border-white/8 bg-[oklch(0.10_0_0)] flex flex-col items-center text-center">
                    ...
                    <h3 className="text-sm font-semibold text-foreground mb-2">Earnings Analytics Coming Soon</h3>
                    <p className="text-xs text-muted-foreground max-w-xs mb-5">Once you complete projects through the hiring system, your earnings will be automatically tracked here.</p>
                    <Link href="/hiring" className="flex items-center gap-1.5 px-4 py-2 rounded-xl gold-gradient text-black text-sm font-semibold hover:opacity-90 transition-opacity">
                      View Contracts <MessageSquare className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                </div>
              )}
